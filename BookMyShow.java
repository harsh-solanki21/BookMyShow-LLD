...
public class BookMyShow {
            ...
            for(Show show : showArray) {
                if (show != null) {
                    if (movieMap.containsKey(show.getMovie().getName())) {
                        ...
                        ArrayList<Show> movieShowList = new ArrayList<Show>();
    ...
    public BookMyShow(ArrayList<Theater> theaters) {
        ...
        this.movieMap = new HashMap<String, ArrayList<Show>>();
        generateMovieMap();
    ...
    public static void main(String[] args) {
        ...
        // Creating Guest User --> Harsh
        GuestUser harsh = new GuestUser("Harsh");

        // Creating Registered User --> Nisarg
        RegisteredUser nisarg = new RegisteredUser("Nisarg");

        // Creating Registered User --> Dhruva
        RegisteredUser dhruva = new RegisteredUser("Dhruva");
        ...
        // Creating Movie object --> Hera Pheri
        Movie herapheri = new Movie("Hera Pheri", Language.HINDI,Genre.COMEDY);
        ...
        try {
            ...
            // Creating Show for Movie HERA PHERI on 7 Jun 2020 @ 12:00 PM in PVR
            dateInString = "Friday, Jun 7, 2020 12:00:00 PM";
            date = formatter.parse(dateInString);
            show2 = new Show(date,herapheri,pvr_gip);
        ...
        ArrayList<Theater> theaterArrayList= new ArrayList<Theater>();
        ...
        */
        // Now suppose NISARG and DHRUVA both wants to book 10 tickets each for first show
        // Then Book My show will create two Ticket Booking threads for their requests

        Show bookingShow = searchedShow.get(0);

        // Ticket Booking Thread for the request made by NISARG for 10 Seats
        TicketBookingThread t1 = new TicketBookingThread(bookingShow,nisarg,10);

        // Ticket Booking Thread for the request made by DHRUVA for 10 Seats
        TicketBookingThread t2 = new TicketBookingThread(bookingShow,dhruva,10);
       ...
       // Start both the Ticket Booking Threads for execution
        ...
        // After execution t1 will carry NISARG ticket and t2 will carry DHRUVA ticket
        Ticket nisarg_ticket = t1.getTicket();
        Ticket dhruva_ticket = t2.getTicket();
        ...
        // Suppose NISARG wants another 15 seats and DHRUVA wants another 10 seats to be booked

        // Ticket Booking Thread for the request made by NISARG for another 15 Seats
        TicketBookingThread t3 = new TicketBookingThread(bookingShow,nisarg,15);

        // Ticket Booking Thread for the request made by DHRUVA for another 10 Seats
        TicketBookingThread t4 = new TicketBookingThread(bookingShow,dhruva,10);
        ...
        // After execution t3 will carry NISARG ticket and t4 will carry DHRUVA ticket
        Ticket nisargNewTicket = t3.getTicket();
        Ticket dhruvaNewTicket = t4.getTicket();

        System.out.println(nisargNewTicket);
        System.out.println(dhruvaNewTicket);

        /* Running this program several times, we will notice that 
        if t3 enters the bookTicket function first, 
        then ticket is allocated to Nisarg, 
        otherwise ticket is allocated to Dhruva.
         */
